...
from MPCControl_base_D6_1 import MPCControl_base
...
class MPCControl_z(MPCControl_base):
    ...
    def plot_invariant_set_evolution(self, plot_dims=(0, 1)):
        ...
        # --- OPACITY: fast fade early, slow near the end ---
        alpha_max = 0.85   # iteration 0
        alpha_mid = 0.10   # around the knee
        alpha_min = 0.02   # last iteration

        knee_frac = 0.20   # first 20% iterations fade a lot (tune this)
        p_in  = 0.35       # <1  => faster fade at the very beginning
        p_out = 3.0        # >1  => slower fade near the end (flattening)
        ...
        def alpha_for(k: int) -> float:
            ...
            if k <= knee:
                # Fast early fade: alpha_max -> alpha_mid
                t = k / knee
                t_ease = t**p_in
                return alpha_max + (alpha_mid - alpha_max) * t_ease
            else:
                # Slow near end: alpha_mid -> alpha_min
                denom = (n - 1 - knee)
                t = (k - knee) / denom if denom > 0 else 1.0
                # ease-out: derivative goes to 0 as t -> 1
                t_ease = 1.0 - (1.0 - t)**p_out
                return alpha_mid + (alpha_min - alpha_mid) * t_ease
    ...
    def min_robust_invariant_set(self, A_cl: np.ndarray, W: Polyhedron, max_iter: int = 50, plot_evolution: bool = True, plot_dims=(0, 1),) -> Polyhedron:
        nx = A_cl.shape[0]
        Omega = W
        self.Omega_hist = []
        ...
        while i <= max_iter:
            ...
            _ = Omega_next.minHrep()
            _ = Omega_next.Vrep         # force V-rep computation
            _ = Omega_next.minVrep()    # VERY IMPORTANT OR ELSE COMPUTATION WILL BE VERY LONG !
